...
$(document).ready(function() {
  ...
  $('a[href*="#"]:not([href="#0"])').click(function(ev) {
	...
	if ($(window).width() > 991) {  
	  ...
	  /* Programming */
	  var $programmingTrigger = $('.programming'),
	  		$programmingVisual = $('.programming__visual');
	  	
	  var programmingScene = new ScrollMagic.Scene({
		  triggerElement: $programmingTrigger,
		  reverse: false
	  })
	  	.setTween(TweenLite.from($programmingVisual, 0.8, {x: 100, opacity: 0, ease: Power1.easeOut}))
	  	.addTo(controller);
	  	
	  /* Kendo */
	  var $kendoTrigger = $('.kendo'),
	  		$kendoVisual = $('.kendo__visual');
	  	
	  var kendoScene = new ScrollMagic.Scene({
		  triggerElement: $kendoTrigger,
		  reverse: false
	  })
	  	.setTween(TweenLite.from($kendoVisual, 0.8, {x: -100, opacity: 0, ease: Power1.easeOut}))
	  	.addTo(controller);
	  	
	  /* Work Experience */
	  var $resumeTrigger = $('.resume'),
	  		$resumeContent = $('.resume__content'),
	  		$resumeVisual = $('.resume__visual'),
	  		$resumeFrame = CSSRulePlugin.getRule('.resume__list:before'),
	  		resumeTl = new TimelineLite();
	  		
	  resumeTl
	  	.set($resumeVisual, {scale: 1})
	  	.from($resumeContent, 0.8, {x: -50, opacity: 0, ease: Power1.easeOut})
	  	.from($resumeVisual, 0.8, {x: 50, opacity: 0, ease: Power1.easeOut}, 0)
	  	.from($resumeFrame, 0.8, {cssRule:{opacity: 0}, ease: Power1.easeOut});
	  	
	  var resumeScene = new ScrollMagic.Scene({
		  triggerElement: $resumeTrigger,
		  offset: 60,
		  reverse: false
	  })
	  	.setTween(resumeTl)
  ...
  $('.resume__navigation-el').click(function() {
	  
	  var $this = $(this),
	  		position = $this.parent().children().index($this);
	  		
	  $this.parent().children().removeClass('resume__navigation-el--is-active');
	  $this.addClass('resume__navigation-el--is-active');
	  $('.resume__list').children().removeClass('resume__list-el--is-active');
	  $('.resume__list').children().eq(position).addClass('resume__list-el--is-active');
